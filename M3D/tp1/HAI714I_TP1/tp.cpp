...
int nX_variant=20,nY_variant=20;
...
//To complete
void setUnitSphere( Mesh & o_mesh, int nX=nX_variant, int nY=nY_variant )
{
    o_mesh.vertices.clear();
    o_mesh.normals.clear();
    o_mesh.triangles.clear();
    float pasX = (2*M_PI)/nX;
   float pasY = (M_PI)/(nY-1);
   Vec3 sommet;
   float x,y,z,angleX,angleY; 
   for (int i  = 0; i < nX; i++){
        for (int j=0; j < nY ; j++){
            angleX = 0 + pasX*i;
            angleY = -(M_PI/2)+pasY*j;
            x = cos(angleX)*cos(angleY);
            y = sin(angleX)*cos(angleY);
            z = sin(angleY);
    ...
    for (int i  = 0; i < nX*nY; i++){
        vertice = o_mesh.vertices[i];
        norm = (float) sqrt(pow(vertice[0],2) + pow(vertice[1],2) + pow(vertice[2],2));
        o_mesh.normals.push_back(Vec3(vertice[0]/norm,vertice[1]/norm, vertice[2]/norm));
    }
    int v0,v1,v2,v3;
    Triangle triangle;
    ...
    //les extremites
    for (int i  = nX; i < nX*nY+1; i+=nX+1){
        v0 = i;
        v1 = i-nX ;
        v2 = v0+nX+1;
        v3 = v1+nX+1;
        std::cout << "v0-v1-v2-v3 " << v0 << "-" << v1 << "-"  << v2 << "-"  << v3<< std::endl;
            o_mesh.triangles.push_back(Triangle(v0,v2,v1));
            o_mesh.triangles.push_back(Triangle(v1,v2,v3));   
        }
...
void key (unsigned char keyPressed, int x, int y) {
    ...
    case '+': //Toggle unit sphere mesh display
        ...
        break;
    case '-': //Toggle unit sphere mesh display
        if (nX_variant > 0 && nY_variant > 0){
            std::cout << nX_variant ;
            std::cout << nY_variant<< std::endl;
            nX_variant --;
            nY_variant --;
            std::cout << nX_variant ;
            std::cout << nY_variant<< std::endl;
