...
class EnveloppeConvexe {
	...
	findNext(previous, min) {
		let V = this.points;
		let gauche = previous != 0 ? 0 : 1 ;
		for (let i = 0; i < V.length; i++) {
			if (previous != gauche && previous != i && gauche != i && this.tour(V[previous], V[gauche], V[i]) == -1){
				gauche = i;
			}
		}
		return gauche;
	...
	algoJarvis(V) {
		...
		//initialisations
		let min = this.findMinIdx(V);
		let envconv = [min];
		let current, previous = min;
		//début
		while (current != min) {
			if (current != null) envconv.push(current);
			current = this.findNext(previous, min);
