...
struct Mesh {
    ...
    void buildColorArray (){
        int size = vertices.size();
        for (int i=0; i< size; i++){
            colorArray.push_back(0.0);
            colorArray.push_back(1.0);
            colorArray.push_back(0.0);
        }
    }
};
...
void drawTriangleMesh( Mesh const & i_mesh ) {
    ...
    glDrawElements(GL_TRIANGLES, mesh.triangleArray.size() , GL_UNSIGNED_INT, ( GLvoid*)(&mesh.triangleArray[0])) ;  
