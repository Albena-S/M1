...
struct TriangleVArray {
    ...
    void initBuffers(){
        ...
        // Utiliser
        // glGenBuffers(...); - genere un identifiant a la position 0
        glGenBuffers(1, &vertexbuffer);
        // glBindBuffer(...);
        glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
        // glBufferData(...); - utilise l'identifiant deja actif -
    ...
    void draw (){
        ...
        //A faire
        //Utiliser glVertexAttribPointer id est 0 deja cree en haut
        //          il faut activer
        glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
        ...
        // 2nd attribute buffer : couleurs - l'id pour les couleurs sera 1
        glBindBuffer(GL_ARRAY_BUFFER, colorbuffer);
        glEnableVertexAttribArray(1); 
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, 0);
        
        //a faire : glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _);

        // Draw the triangle !
        // Utiliser glDrawArrays - count cest le triplet des coordonnes parce qu'on a specifie GL_TRIANGLE
...
void draw () {
    ...
    int scale_id = glGetUniformLocation(programID, "scale");
    int translate_id = glGetUniformLocation(programID, "translate");
    //                          GPU         CPU
    ...
    glUniform3fv( translate_id ,1, &translate.mVals[0]);
...
void key (unsigned char keyPressed, int x, int y) {
    ...
    case '+': //Press + key to increase scale
        scale += .005;//Completer augmenter la valeur de la variable scale e.g. +0.005
        break;
    ...
    case 'q': //Press q key to translate on x negative
        translate.mVals[0] -= .2;//Completer : mettre à jour le y du Vec3 translate
        break;

    case 'z': //Press z key to translate on y positive
        translate.mVals[1] += .2;//Completer : mettre à jour le y du Vec3 translate
        break;
