...
//t - pour trier les intersections le plus proches
//resultat de la formule r(t) = p0 + t*d
    float t;
//coord spherique - aident pour les textures
    float theta,phi;
    Vec3 intersection;
...
public:
    ...
    RaySphereIntersection intersect(const Ray &ray) const {
        ...
        a = Vec3::dot(d,d);
        b = 2. * Vec3::dot(d, (o_c));
        c = o_c.length() * o_c.length() - m_radius*m_radius;
        discr = b*b - 4*a*c;

        //discriminant 
        //2 racines - prends le plus petit t
        if (discr > 0){
            int t1 = (-b + sqrt(discr)) / 2*a;
            int t2 = (-b - sqrt(discr)) / 2*a;
            t = t1 < t2 ? t1 : t2; 
        }else if(discr == 0){
            //1 racine 
            t = -(b/(2*a));
        }else{
        //pas de racine - intersect pas
